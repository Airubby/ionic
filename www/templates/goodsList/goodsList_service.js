  ...
  .factory('GoodsListFty', function($http, $q) {
    return {
    	 ...
    	 refreshGoodsList:function(message){
    	 	var goodsMessage = JSON.parse(message);
    	 	//console.log(message)
    	 	if(goodsMessage.typeNumber==10001&&goodsMessage.pageNum==1){
    	 ...
    	 loadMoreGoodsList: function (message) {
    	 	var goodsMessage = JSON.parse(message);
    	 	if(goodsMessage.typeNumber==10001&&goodsMessage.pageNum>1&&goodsMessage.pageNum<4){
    	 		var obj_goodsListData = [
	          {
	            name: '澳贝琳 2015秋冬新款韩版修身显瘦中长款毛呢大衣女外套  8615  灰色 L',
	            price: '198',
